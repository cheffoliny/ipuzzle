...
Class kmz {
	...
	function __construct() 
	{
		...
		$oStyle = $dom->createElement( "Style" );
		$oStyle->setAttribute( "id", "my_line_style" );
		$oStyleNode = $doc->appendChild( $oStyle );
		
		$oLineStyle = $dom->createElement( 'LineStyle' );
		$oLineStyleNode = $oStyleNode->appendChild( $oLineStyle );
		
		$oColorNode = $dom->createElement( 'color', "fff44444" );
		$oLineStyleNode->appendChild( $oColorNode );
		$oColorModeNode = $dom->createElement( 'colorMode', "normal" );
		$oLineStyleNode->appendChild( $oColorModeNode );
		$oWidthNode = $dom->createElement( 'width', "2" );
		$oLineStyleNode->appendChild( $oWidthNode );
...
//		)
	...
	public function addLine( $oFolderNode, $sName, $aCoords1, $aCoords2 )
	{
		$dom = &$this->dom;
		
		$sCoords = implode( ",", $aCoords1 ) . ",0 " . implode( ",", $aCoords2 ) . ",0";
		
		$oNode = $dom->createElement( "Placemark" );
		$oPlaceNode = $oFolderNode->appendChild( $oNode );
		
		$oNameNode = $dom->createElement( 'name', $sName );
		$oPlaceNode->appendChild( $oNameNode );
		$oStyleNode = $dom->createElement( 'styleUrl', "#my_line_style" );
		$oPlaceNode->appendChild( $oStyleNode );
		
		$oLine = $dom->createElement( 'LineString' );
		$oLineNode = $oPlaceNode->appendChild( $oLine );
		
		$oTessellateNode = $dom->createElement( 'tessellate', "1" );
		$oLineNode->appendChild( $oTessellateNode );
		$oCoordsNode = $dom->createElement( 'coordinates', $sCoords );
		$oLineNode->appendChild( $oCoordsNode );
