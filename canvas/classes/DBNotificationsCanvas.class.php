...
class DBNotificationsCanvas extends DBBase2 {
	...
	*/
	...
	function addObjectFeePersonNotification($nIDPerson, $nIDObject, $aContract, $fee_type) {
		...
		if(!empty($aObjectFeePersonsEvent['sms']) && !empty($user_phone)) $event_channels[] = 'sms';
		if(!empty($aObjectFeePersonsEvent['mail']) && !empty($user_email)) $event_channels[] = 'mail';
		if(!empty($aObjectFeePersonsEvent['system'])) $event_channels[] = 'system';
