...
try {
	...
	if($aRequest) {		
		...
		else 
		{
			if($layer_type == 'object') {
				...
				if(empty($id_contract))
				{
					...
					// ако е изграждане взимам от договора инфомацията
					$sQuery = "
						SELECT 
						  c.id AS id_object,
						  c.id_office,
						  c.obj_geo AS obj_geo,
						  c.obj_name AS obj_name
						FROM
						  $db_name_finance.contracts AS c
						WHERE c.id = $id_contract ";

					$aObjFromContract = $oDB->select($sQuery);
					$aObjFromContract = $aObjFromContract[0];
					
					$aTmp = explode(',' ,$aObjFromContract['obj_geo']);
					
					$aObjFromContract['obj_geo_lat'] = trim($aTmp[0]);  
					$aObjFromContract['obj_geo_lan'] = trim($aTmp[1]);  
					...
					$aRegister = $oDB->select($sQuery);
					$aRegister = $aRegister[0];
					
					$aObjFromContract['start_time'] = $aRegister['start_time'];
					
					$result['object'] = $aObjFromContract;
				}
		...
		if(!empty($id_alarm)) {
			$sQuery .= " WHERE ah.id_tech_register = $id_alarm";		
		} else {
			$sQuery .= " WHERE ah.id_tech_register = $id_alarm_patrul";		
		}
