...
pl.init				= function(){
	...
	//ID_ALARM=193;
	if (ID_ALARM || ID_ALARM_PATRUL) pl.serverCall({id_alarm:ID_ALARM,id_alarm_patrul:ID_ALARM_PATRUL,layer_type:LAYER_TYPE},pl.draw);
}
...
pl.play				= function(start,end,clear,auto,stop) {
	...
	var render = function() {
		...
		for(;start<=end;start++) {		
			currentFrame = start;
            console.log(oRegion.events[start].patrul_geo_lat + ' => ' + oRegion.events[start].patrul_geo_lan + ' => ' + oRegion.events[start].alarm_status);

            //directionsDisplay.setMap(pl.bgMap);
/*
            var request = {
                origin: new google.maps.LatLng(oRegion.events[start].patrul_geo_lat,oRegion.events[start].patrul_geo_lan),
                destination: new google.maps.LatLng(oRegion.object.obj_geo_lat,oRegion.object.obj_geo_lan),
...
*/
			...
			var state;
			switch (oRegion.events[start].alarm_status) {
				case "alarm":
				...
				break;
				case "position":
					state = "Патрул "+oRegion.events[start].id_patrul+" позиция ";
				break;
				case "start":
...
pl.gMaps			= function() {
    ...
    directionsDisplay.setMap(bgMap);
}
...
/*
pl.calcRoute = function() {
    //var start = document.getElementById("start").value;
    //var end = document.getElementById("end").value;
    //var distanceInput = document.getElementById("distance");

    var request = {
        origin: new google.maps.LatLng(oRegion.zoomBounds.s,oRegion.zoomBounds.w),
        destination:end,
        travelMode: google.maps.DirectionsTravelMode.DRIVING
    };
