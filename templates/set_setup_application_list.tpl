			...
			<div class="row mb-1">
				<div class="col ml-3">
					<div class="input-group input-group-sm">
						...
						<button class="btn btn-sm btn-primary mx-1" type="button" onClick="loadXMLDoc('result'); return false;" name="Button"><span class="ui-icon ui-icon-search" aria-hidden="true"></span>Търси</button>
						<button class="btn btn-sm btn-success" id="b100" onClick="setApplication(0);"><span class="ui-icon ui-icon-plus" aria-hidden="true"></span>Добави</button>
					</div>
				</div>
			</div>

			<div id="result" class="ui-personnel-result" rpc_excel_panel="off" rpc_paging="off" rpc_resize="off" style="height:350px; overflow: auto;"></div>
			<!-- край на работната част -->
			<nav class="navbar fixed-bottom flex-row mb-2 py-0 navbar-expand-lg py-md-1" id="search">
				<div class="col-12">
					<div class="input-group input-group-sm">
						<button class="btn btn-block btn-sm btn-danger" onClick="parent.window.close();" ><span class="ui-icon ui-icon-close" aria-hidden="true"></span> Затвори</button>
					</div>
				</div>
			</nav>
		</div>
