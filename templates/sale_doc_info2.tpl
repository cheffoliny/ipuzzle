...
//			return false;
		...
		function finalDoc() {
			...
			rpc_on_exit = function() {
				...
				if ( url.length > 0 ) {
					parent.opener.formSubmit();
					url = url + '&doc_type=sale&id=' + $('nID').value;
					window.location=url;
...
<form id="form1" action="" onsubmit="return false">
	...
	<div id="page_caption" class="page_caption" {if $new}style="background: green"{/if}>{$sPageCaption|default:''}</div>

	<table width="100%" cellpadding="0" cellspacing="0" id="filter">
		<tr>
			<td>
				{if $nID}{include file='sale_doc_tabs.tpl'}{/if}
			</td>
		</tr>
	</table>
