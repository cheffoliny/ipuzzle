...
switch ($aParams['api_action']) {
    ...
    default:
        ...
        if( empty( $aDuplicate ) )
        {
            ...
            $aLeave['remaining_days'] 	= $oDBPersonLeaves->getHowManyDaysToAdd( $aLeave['id_person'] );
            $aLeave['type'] 		= "leave";
        ...
        if ( ($nResult = $oHospital->getResult($aParams['id'], $aParams['sfield'], $aParams['stype'], $aParams['current_page'], $oResponse)) != DBAPI_ERR_SUCCESS ) {
            ...
            $oResponse->setField( 'due_days', 			'полагаеми', 			'Сортирай по дни' );
            $oResponse->setField( 'used_days',			'използвани',			'Сортирай по дни' );
            $oResponse->setField( 'remaining_days', 	'оставащи за година', 			'Сортирай по дни' );
//                $oResponse->setField( 'remain',				'оставащи', 			'Сортирай по дни' );
//				$oResponse->setField( 'due_days_all', 		'полаг. общо', 				'Сортирай по дни' );
            $oResponse->setField( 'used_extra_days',	'изп. допълн.',			'Сортирай по дни' );
        ...
        foreach( $oResponse->oResult->aData as $nKey => $aValue )
        {
            ...
            if($aValue['year'] < $year -2 && $aValue['year'] != 2009) {
                $oResponse->setDataAttributes( $nKey, "remaining_days", array( "style" => "color: red;", "title" => "Дните не се зачитат!") );
            }
            APILog::Log(0,ArrayToString($aValue));
        }
