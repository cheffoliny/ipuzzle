<?php
		...
		while ( $rowPay = @mysql_fetch_assoc($resPay) ) {
			...
			$insQuery = "
				...
				SELECT 
					...
					invoice_recipient, fdds.jur_name, fdds.address, fdds.idn, fdds.idn_dds, fdds.jur_mol, '{$sum}', 'yслуга', '{$order_sum}', 0, 
			...
			if ( strlen($nIDSaleDoc) != 13 ) {
				...
				$insQuery = "
					...
					SELECT
						{$nIDSaleDoc}, o.id_office, o.id, '{$paid_month}', ns.id, 0, '{$id}', ns.name, CONCAT('[', o.num, '] ', o.invoice_name), 1, 'бр.', '{$sum}' / 1.2, '{$sum}' / 1.2, '{$order_sum}' / 1.2, NOW(), 0, 35, NOW()
					FROM {$sod}.objects o
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		$nIDSer		= is_numeric($row['id_service']) ? $row['id_service'] : 0;
		...
		$insQuery = "
			...
			SELECT o.id, o.id_office, ns.id, ns.name, '{$price}', 1, '{$price}', '{$start}', '{$paid}', 35, NOW(), 0 
			FROM {$sod}.objects o, {$finance}.nomenclatures_earnings n 
			LEFT JOIN {$finance}.nomenclatures_services ns ON ns.id_nomenclature_earning = n.id
			WHERE o.id_oldobj = {$nID}
				AND ns.is_schet = 1
	...
	while ($row = mysql_fetch_assoc($result12)) {
		...
		$insQuery = "
			...
			SELECT o.id, o.id_office, ns.id, '{$nIDService}', ns.name, '{$price}', 1, '{$price}', '{$start}', '{$paid}', 35, NOW(), 0 
			FROM {$sod}.objects o, {$finance}.nomenclatures_earnings n 
			LEFT JOIN {$finance}.nomenclatures_services ns ON ns.id_nomenclature_earning = n.id
			WHERE o.id_oldobj = {$nID}
				AND ns.is_schet = 1
