				...
				if ( !empty($nIDClient) ) {
					...
					$insQuery = "
						...
						VALUES 
							...
							'{$cl_recp}', 'Инфра ЕООД', 'гр. Димитровград, ул. Хр. Г. Данов 10-Б3', '111111111', 'BG 111111111', 'Любомир Ненчев Гочев', '{$sum}', 'yслуга', '{$order_sum}', 0, 
				...
				echo $insQuery;
					...
					if ( strlen($nIDSaleDoc) != 13 ) {
						...
						$insQuery = "
							...
							SELECT
								{$nIDSaleDoc}, 67, 0, '{$paid_month}', ns.id, 0, '{$id}', {$info}, '', 1, 'бр.', '{$sum}' / 1.2, '{$sum}' / 1.2, '{$order_sum}' / 1.2, NOW(), 0, 35, NOW()
