...
function _convertRange2d($range)
 {
    ...
    if (preg_match('/^\$?([A-Ia-i]?[A-Za-z])\$?(\d+)\:\$?([A-Ia-i]?[A-Za-z])\$?(\d+)$/',$range)) {
        list($cell1, $cell2) = explode(':', $range); //split(':', $range);
    }
...
function _convertRange3d($token)
 {
    ...
    list($ext_ref, $range) = explode('!', $token); //split('!', $token);
...
function _convertRef3d($cell)
 {
    ...
    // Split the ref at the ! symbol
    list($ext_ref, $cell) = explode('!', $cell); //split('!', $cell);
...
function _packExtRef($ext_ref) {
 ...
 {
        list($sheet_name1, $sheet_name2) = explode(':', $ext_ref); // split(':', $ext_ref);
