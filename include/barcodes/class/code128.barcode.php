 ...
 *--------------------------------------------------------------------
 * $Id: code128.barcode.php,v 1.11 2006/02/10 02:15:44 jsgoupil Exp $
 *--------------------------------------------------------------------
...
class code128 extends BarCode {
	...
	public function getMaxWidth() {
		...
		for ($i = 0; $i < $c; $i += $increment) {
			$index = $this->findIndex($this->text[$i]);
			if ($index === 99) {
				$increment = 2;
			} elseif ($index === 100 || $index === 101) {
				$increment = 1;
			}
	...
	protected function calculateChecksum() {
		...
		for ($position = 1, $i = 0; $i < $c; $position++, $i++) {
			if ($this->currentCode === 'C') {
				...
				$this->checksumValue += $this->findIndex($this->text[$i]) * $position;
			}
