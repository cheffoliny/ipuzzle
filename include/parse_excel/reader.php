...
class Spreadsheet_Excel_Reader {
    ...
    */
    ...
    function read($sFileName, $content=false) {
    	
    	$errlevel = error_reporting();
    	error_reporting($errlevel ^ E_NOTICE);
        $res = $this->_ole->read($sFileName, $content);
        ...
        if($res === false) {
        	...
        	if($this->_ole->error == 1) {
        		// bad file
        		$this->_errMsg = DBAPI_ERR_WORNG_FILE;
        		// check other error codes here (eg bad fileformat, etc...)
        	}
        }
		
        if ($this->_errMsg == DBAPI_ERR_SUCCESS) {
