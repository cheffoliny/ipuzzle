...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<script type="text/javascript" language="javascript">		
		...
		function <xsl:value-of select="$rpc_prefix"/>setPointer(theRow,theRowNum,id_row) {
			...
			<![CDATA[
				...
				for(i=1; i < rowsLength; i++) {
					...
					var row_color = i % 2 ? 'white' : '#F0F0F0';

					var notEdit = crow.getAttribute('not-edit-color');
				...
				for ( i=0 ; i<theRow.cells.length ; i++ ) {
					...
					theRow.cells[i].style.fontWeight = 'bold';
				}
				
				
				document.getElementById('id_request').value = id_row;
				parent.document.getElementById('id_request').value = id_row;
			]]>
			</xsl:text>
					...
					| страница 
							...
							<xsl:choose>
								<xsl:when test="$current_page != 1">
									<a href="#" onClick="document.getElementById('{$rpc_prefix}current_page').value = 1; return {$rpc_prefix}xslLoadXML()">
										първа
									</a>
								</xsl:when>
								<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="$current_page != 1">
									<a href="#" onClick="document.getElementById('{$rpc_prefix}current_page').value--; return {$rpc_prefix}xslLoadXML()">					
										предишна
									</a>
								</xsl:when>
								<xsl:otherwise>
							...
							<xsl:choose>
								<xsl:when test="$current_page != $last_page">
									<a href="#" onClick="document.getElementById('{$rpc_prefix}current_page').value = {$last_page}; return {$rpc_prefix}xslLoadXML()">
										последна
									</a>
								</xsl:when>
								<xsl:otherwise>
	...
	<xsl:template name="operation">
		...
		<div height="40">
			...
			<table cellspacing="0" cellpadding="5">
				<tr>
					<td>
						<img src="images/arrow_up.gif" border="0"/><img src="images/line_middle.gif"/>
					</td>
					<td>	
					...
					<td>	
						<button id="{concat($rpc_prefix,'rpc_btn_action')}" name="{concat($rpc_prefix,'rpc_btn_action')}" style="width:80px"><xsl:attribute name="onClick"><xsl:value-of select="$rpc_prefix"/>just_do_it();</xsl:attribute>изпълни</button>
					</td>
				</tr>
			</table>
		</div>
