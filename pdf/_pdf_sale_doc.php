...
class SaleDocPDF extends PDFC {
    ...
    var $aMargin = array( "left"=>20, "top"=>15, "right"=>10, "bottom"=>18 );
    var $aOptions = array(
        'TextColor'		=>array('r'=>0,'g'=>0,'b'=>0),
        'Background'	=>array('title'=>245,'body'=>245),
        'BorderColor'	=>255,
        'BorderWidth'	=>0.55,
        'FontSize'		=>7.2,
        'RowHeight'		=>5,
    ...
    public function SaleDocPDF($orientation) {
    ...
    private function printLogo( $aSaleDoc ) {
        ...
        $this->Image( $_SESSION['BASE_DIR'].'/images/'.$fEIN.'.png',0,10,185);
    }
...
//							'jpg' );		
    ...
    private function PrintTotal($aSaleDoc) {
        ...
        $this->Cell( $width / 2,  $this->aOptions['RowHeight'], "Съставил : ".$sCreatedUser, 1, 0, 'L', 1);
    ...
    public function PrintReport( $nID, $sPrintType = '', $sViewType = 'single', $local_save = 0 ) {
