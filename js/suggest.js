...
var suggestDebug = true;		//TODO: в крайната версия трябва да стане 0!

var selectedSuggestObject = null;
var xmlHTTP = null;
var queryURL = "api/api_suggest.php";
var suggest_elements = new Array();

//Ползва се за инициализиране на suggest елементи
var InitSuggestForm = function(){};


if (window.addEventListener){
	window.addEventListener("load", InitSuggests, false);
} else if (window.attachEvent){
	window.attachEvent("onload", InitSuggests);
}
...
Suggest.prototype = {
	initialize: function()
	{
		...
		this.DIV_BG_COLOR = "#ffffcc";
		this.DIV_HIGHLIGHT_COLOR = "#DAE2F5";
		this.listStyle = "";
		this.listItemStyle = "";
		this.listItemKey = "";
		this.listItemValue = "";
	...
	InitQueryCode: function(name, queryFieldName)
	{
		...
		this.queryField.onkeyup = suggestKeyUpHandler;
		...
		this.addToCache("", new Array(), new Array(), new Array());
	}
	,
	...
	/**
	 ...
	 repeat lookups.
	 */
	addToCache: function(queryString, resultArray1, resultArray2, resultArray3)
	{
		this.cache[queryString] = new Array(resultArray1, resultArray2, resultArray3);
	}
	,
		...
		if (!this.globalDiv)
		{
			...
			if (!this.divFormatted)
			{
				...
				if (this.listStyle == "")
				{
					...
					this.globalDiv.style.fontSize = "80%";
					this.globalDiv.style.padding = "1px";
				...
				this.globalDiv.style.zIndex = 50;
	 ...
	 */
	showQueryDiv: function(queryString, resultArray1, resultArray2,  resultArray3)
	{
		...
		for (var i = 0; i < resultArray1.length; i++)
		{
			...
			else
			{
				...
				result.style.borderBottom = "1px solid #B5B5B5";
			...
			if (this.listItemValue == "")
			{
				...
				result2.style.paddingLeft = "10px";
			}
			result2.innerHTML = resultArray2[i];

			// tolito edit
			result2.style.whiteSpace = 'nowrap';
			var tolito_div = document.createElement('textarea');
			tolito_div.style.display = 'none';
			tolito_div.value = resultArray1[i] + "¤" + resultArray2[i] + "¤" + resultArray3[i];


			//var result3 = document.createElement("span");
			//result3.className = resultArray1[i] + ";" + resultArray2[i] + ";" + resultArray3[i];


			//result.appendChild(result3);
			result.appendChild(tolito_div);
		...
		if (!isCached)
			this.addToCache(queryString, resultArray1, resultArray2, resultArray3);
	...
	{
		e.stopPropagation();
		selectedSuggestObject.queryField.blur();
	...
	{
		var spans = item.getElementsByTagName("textarea");

		if (spans) {

			var data = spans[0].value.split("¤");
			...
			if (this.queryFieldHolds == 0) {
				...
				//value
				this.queryField.value =  data[2];
			} else if (this.queryFieldHolds == 2) {
				//both key and value
				this.queryField.value = data[0] + " - " + data[2];
			}
...
var timeout;
function suggestKeyUpHandler(evt)
{
	if(!evt && window.event)
		evt = window.event;
	...
	if ((key != KEYUP) && (key != KEYDOWN) && (key != KEYENTER) && (key != KEYTAB)) {
		...
		timeout = setTimeout("suggestMainLoop()",400);
	}
...
{
	val = selectedSuggestObject.queryField.value;

	if( !selectedSuggestObject.queryField.readOnly && ( val.length >= selectedSuggestObject.minCharToStartSearch ) )
	{
		...
		if (selectedSuggestObject.lastVal != val && selectedSuggestObject.searching == false)
		{
			...
			if (cacheResult)
				selectedSuggestObject.showQueryDiv(val, cacheResult[0], cacheResult[1], cacheResult[3]);
			else
			...
			if (XMLHttpRequest) {
				...
				xmlHttp.onreadystatechange = function()
				{
					if( xmlHttp.readyState == 4 && xmlHttp.status < 500 ) {
						...
						for(var i=0;i<html_value.length;i++)
						{
							var nPos = html_value[i].toLowerCase().indexOf( val.toLowerCase() );

							if( nPos != -1 )
								html_value[i] = html_value[i].slice(0, nPos) + html_value[i].substr(nPos, val.length).bold() + html_value[i].slice(nPos + val.length);
						}

						if( (value.length == 1) && ( val.toLowerCase() == value[0].toLowerCase() )  )
							selectedSuggestObject.showQueryDiv("", new Array(), new Array(), new Array());
						else
							selectedSuggestObject.showQueryDiv("", key, html_value, value);
					};
				}
				url=queryURL+"?val="+ encodeURIComponent(val)+"&query_type="+selectedSuggestObject.queryType;

				var params = selectedSuggestObject.queryParams.split(";");
				for (var i=0; i<params.length;i++) {
					if( par = document.getElementById(params[i]) )
						if(par.type == 'checkbox') {
							url += "&"+params[i]+"="+par.checked;
						} else {
							url += "&"+params[i]+"="+par.value;
						}
				}
	...
	{
		selectedSuggestObject.showQueryDiv( "", Array() , Array(), Array() );
	}
...
function InitSuggests() {
	...
	for (var i=0;i<inputs.length;i++) {
		if ( inputs[i].getAttributeNode("suggest") ) {

			inputs[i].style.backgroundColor = '#ffffcc';
			var el = new Array ();
			el['id']=inputs[i].id;
			el['suggest']=new Suggest();
			if ( inputs[i].getAttributeNode("queryType") ) {
				el['suggest'].queryType=inputs[i].getAttribute("queryType");
			}
			if ( inputs[i].getAttributeNode("queryParams") ) {
				el['suggest'].queryParams=inputs[i].getAttribute("queryParams");
			}
			next=suggest_elements.length;
			suggest_elements[next] = el;

			suggest_elements[next]['suggest'].InitQueryCode(el['id'], el['id']);
			onSuggestFieldFocus(suggest_elements[next]['suggest']);

			if (inputs[i].addEventListener){
				eval("inputs["+i+"].addEventListener('focus', function () { onSuggestFieldFocus( suggest_elements["+next+"]['suggest'] );}, false);");
			} else if (inputs[i].attachEvent){
				eval("inputs["+i+"].attachEvent('onfocus', function () { onSuggestFieldFocus( suggest_elements["+next+"]['suggest'] );});");
			}
		}
	}
	InitSuggestForm();
