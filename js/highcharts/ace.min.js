...
ace.handle_side_menu = function (a) {
    ...
    var is_sidebar_min = a("#sidebar").hasClass("menu-min");
    a("#sidebar-collapse").on(ace.click_event, function () {
        is_sidebar_min = a("#sidebar").hasClass("menu-min");
        ace.settings.sidebar_collapsed(!is_sidebar_min)
    });
    var nav_user_agent = navigator.userAgent.match(/OS (5|6|7)(_\d)+ like Mac OS X/i);
    a(".nav-list").on(ace.click_event, function (h) {
        var this_link = a(h.target).closest("a");
        if (!this_link || this_link.length == 0) {
            return
        }
        is_sidebar_min = a("#sidebar").hasClass("menu-min");
        if (!this_link.hasClass("dropdown-toggle")) {
            if (is_sidebar_min && ace.click_event == "tap" && this_link.get(0).parentNode.parentNode == this) {
                var i = this_link.find(".menu-text").get(0);
                if (h.target != i && !a.contains(i, h.target)) {
                    return false
                }
            }
            if (nav_user_agent) {
                document.location = this_link.attr("href");
                return false
            }
            return
        }
        var ul_submenu = this_link.next().get(0);
        if (!a(ul_submenu).is(":visible")) {
            var d = a(ul_submenu.parentNode).closest("ul");
            if (is_sidebar_min && d.hasClass("nav-list")) {
                return
            }
            d.find("> .open > .submenu").each(function () {
                if (this != ul_submenu && !a(this.parentNode).hasClass("active")) {
                    a(this).slideUp(200).parent().removeClass("open")
                }
            })
        } else {
        }
        if (is_sidebar_min && a(ul_submenu.parentNode.parentNode).hasClass("nav-list")) {
            return false
        }
        a(ul_submenu).slideToggle(200).parent().toggleClass("open");
        return false
    })
};
